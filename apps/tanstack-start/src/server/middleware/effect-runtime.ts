 ...
 *
 * @example Adding the Database (requires HYPERDRIVE binding in wrangler.jsonc):
 * ```typescript
 * import { Database, connect } from "@repo/db"
 *
 * const dbLayer = Layer.scoped(Database, connect(env.HYPERDRIVE.connectionString))
...
export const effectRuntimeMiddleware = createMiddleware().server(
  async ({ next }) => {
    ...
    //
    // Example with the Database (uncomment when HYPERDRIVE is configured):
    // const dbLayer = Layer.scoped(Database, connect(env.HYPERDRIVE.connectionString))
    // const servicesLayer = Layer.mergeAll(dbLayer, otherServiceLayer)
